...
3. **実装後は動作確認の上、コミットからプッシュまでをClaude Code側で完結させる**
   （2026-08-19変更。従来は「pushはあつのりさんに依頼」としていたが、これは
   **統括Claude側のサンドボックスにGitHub資格情報が無い**という別環境の事情を
   誤って一般化した記述だった。Claude CodeはWindows上でユーザーのgit設定を
   そのまま使えるためpushできる）
   - **push後は必ず `git rev-list --count "@{u}..HEAD"` の実際の出力を確認すること**
     （0件なら完了。「pushしました」という報告ではなく出力で確かめる）
   - 失敗した場合は勝手に再試行や別手段を試さず、エラー全文を添えて報告すること
...
### ★着手前に、指示書が途中で切れていないか確認する

指示はコピー＆ペーストで渡されるため、**途中で切れることがあります。**
あなたから見て「短い指示」と「切れた指示」は区別がつきません。

> **着手前に、指示書に次の6つの節が揃っているか確認すること。**
> **背景／スコープ／やること／禁止事項／完了条件／コミットメッセージ。**
> **1つでも欠けていたら実装を始めず「◯◯が見当たりません」と報告する。**
> 文が途中で終わっている、見出しだけで中身が無い、**参照先の節が存在しない**場合も同じ。

**★併せて**：節を書き換えた指示が来たら、**その節を写した場所（UIの文字列・README等）も直すこと。**
他プロジェクトでは、更新済みの節を参照した実装が古い文言のまま残り、
**実装できない約束が画面に出ていた**という事故が起きています。

### モデルの分担

| 役 | モデル | 理由 |
|---|---|---|
| 統括のClaude（設計・判断・レビュー） | Opus | 仕様の穴を見つける／トレードオフを判断する |
| **Claude Code（あなた・実装・コミット）** | **Sonnet** | 指示書があり、テストと画面で気づける |

`.claude/settings.local.json` の `"model": "sonnet"` で固定しています。
**例外：書き込み・削除・移行など「間違えてもその場では何も起きない」工程だけ、
指示側が実行時に `--model opus` を指定します。**あなたから変更する必要はありません。
...
- バージョン：**v1.1.0（versionCode 68）** をリリース準備中
  （サムネイルキャッシュの枚数と内訳を設定画面に表示。コミット 4e06229）
- **公開中は v1.0.1（versionCode 67）**。2026-09-07 に提出し 09-08 に公開済み
  （サムネイル生成のOOMクラッシュ修正）
- **2026-09-05 に v1.0.0 で Google Play の製品版として正式リリース。
  配信範囲は全世界177か国／地域**
- 英語（en-US）のストア掲載情報・リリースノートとも登録済み
- クローズドテストのトラックは残してある（製品版と併存）
- **★アップデートは「公開中のアプリを差し替える」作業。
  次にAABを上げるときは versionCode を69以上にすること**
- **★R8（縮小・難読化）の有効化が期限付きで残っている（2027年2月・回覧板 T-018）**
