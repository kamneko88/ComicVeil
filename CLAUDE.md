...
- バージョン：**v1.0.1（versionCode 67）** をリリース準備中
  （サムネイル生成のOOMクラッシュ修正。コミット 00bc29f）
- **2026-09-05 に v1.0.0（versionCode 66）で Google Play の製品版として
  正式リリース済み・公開中。配信範囲は全世界177か国／地域**
- 2026-09-06 に英語（en-US）のストア掲載情報を追加し、同日公開済み
- クローズドテストのトラックは残してある（製品版と併存）
- **★これ以降のアップデートは「公開中のアプリを差し替える」作業になる。
  次にAABを上げるときは versionCode を67以上にすること。
  段階的公開（rollout %）の活用を検討する**
